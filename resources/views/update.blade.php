...
<html lang="{{ app()->getLocale() }}">
    ...
    <body>
    	<script>
	    	var app = angular.module('addListing', []);
	    		app.controller('patternController', function($scope) {
      			...
      			$scope.province = "{{$listing->province}}";
      			$scope.telephone = "{{$listing->telephone}}";
      			$scope.postcode = "{{$listing->postcode}}";
      			$scope.salary = "{{$listing->salary}}";
      		});
	    </script>
		...
		<div class="container">
	    <form ng-app="addListing" ng-controller="patternController" name="listingForm" action="/api/listings/{{$listing->id}}" method="post" novalidate>
	        {!! csrf_field() !!}
	        {{ method_field('PUT') }}
